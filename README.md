...
`dist\ClaudeUsageOverlay.exe`가 나온다(약 19MB). 파이썬도 의존성도 필요 없고,
...
저장할 때 `overlay_visible`만 덮어쓰므로 고쳐둔 값이 사라지지는 않는다.

**창 위치는 저장하지 않는다.** 드래그해서 옮긴 자리는 그 세션에서만 유지되고, 다시 켜면
늘 화면 오른쪽 아래(작업 표시줄 위)에 뜬다.
